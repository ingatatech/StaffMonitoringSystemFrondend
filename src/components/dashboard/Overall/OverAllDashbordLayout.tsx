...
import OverAllDashboardSideNav from "./OverAllDashboardSideBar";
import OverAllDashboardNavbar from "./OverAllDashboardNavbar";
import React, { useEffect, useState } from "react";
import { useDispatch } from "react-redux";
import { logout } from "../../../Redux/Slices/LoginSlices";
import axios from "axios";
function OverAllDashboardLayout() {
  ...
  return (
      <div className="bg-gray-100 dark:bg-gray-900 flex flex-col h-screen overflow-hidden">
        ...
        <div className="fixed top-0 left-0 right-0 z-50">
          <OverAllDashboardNavbar toggleSidebar={toggleSidebar} />
        </div>
        ...
        <div className="flex flex-1 pt-16 h-full">
          ...
          <div className={`fixed inset-y-0 left-0 z-40 pt-16 h-full lg:translate-x-0 transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <OverAllDashboardSideNav isOpen={sidebarOpen} closeSidebar={() => setSidebarOpen(false)} />
          </div>
...
export default OverAllDashboardLayout;