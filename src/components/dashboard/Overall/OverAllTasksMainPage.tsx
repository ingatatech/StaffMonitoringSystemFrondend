...
import  { ReviewStatus } from "../SuperVisor/DailyTaskGroup"
import DailyTaskGroup from "./DailyTaskGroup"
import TaskStatistics from "../SuperVisor/TaskStatistics"
...
const OverAllTasksMainPage: React.FC = () => {
  ...
  // Helper function to check if a daily task has active tasks
  const hasActiveTasks = (dailyTask: any) => {
    ...
    const activeTasks = dailyTask.tasks.filter((task: any) => {
      ...
      const isApproved = task.review_status === ReviewStatus.APPROVED
  ...
  const shouldDisplayDailyTask = (dailyTask: any) => {
    ...
    const submissionDateStr = isValidDate
    ...
    // Check if there are any in-progress tasks for today
    const hasInProgressTasks = dailyTask.tasks?.some(
      (task: any) => task.status === "in_progress"
    )
    

    return (!isSubmitted && !isPastDate) || (isToday && hasInProgressTasks)
  }
  ...
  // Separate calculation for tasks that should show components vs NoComponentDailyTask
...
export default OverAllTasksMainPage