...
// ENHANCED: Same date utilities as SupervisorMainPage for consistency
...
const DailyTaskGroup: React.FC<DailyTaskGroupProps> = ({ dailyTask }) => {
  console.log(`🔍 [DailyTaskGroup] Processing daily task ${dailyTask.id}`)
  console.log(`🔍 [DailyTaskGroup] Submission date: ${dailyTask.submission_date}`)
  console.log(`🔍 [DailyTaskGroup] Submitted: ${dailyTask.submitted}`)
  console.log(`🔍 [DailyTaskGroup] Total tasks: ${dailyTask.tasks?.length || 0}`)
  ...
  // ENHANCED: Safe date calculations with proper error handling
  ...
  console.log(`🔍 [DailyTaskGroup] Date analysis:`, {
    today,
    submissionDateStr,
    isToday,
    isPastDate,
    submitted: dailyTask.submitted
  })

  // ENHANCED: Fixed filtering logic for tasks within this daily task group
  // This is the KEY FIX - we need to show completed tasks that are NOT yet submitted
  // and have pending review status (not approved)
  const filteredDailyTasks = useMemo(() => {
    console.log(`🔍 [DailyTaskGroup] Filtering tasks for daily task ${dailyTask.id}`)
    console.log(`🔍 [DailyTaskGroup] Daily task submitted status: ${dailyTask.submitted}`)

    if (!dailyTask.tasks || dailyTask.tasks.length === 0) {
      console.log(`🔍 [DailyTaskGroup] No tasks found`)
      return []
    }
    ...
    const filtered = dailyTask.tasks.filter((task) => {
      ...
      // MAIN CONDITION: Show tasks that are:
      // 1. NOT completed, OR
      ...
      console.log(`🔍 [DailyTaskGroup] Task "${task.title}":`, {
        status: task.status,
    ...
    console.log(`🔍 [DailyTaskGroup] Filtered tasks: ${filtered.length} out of ${dailyTask.tasks.length}`)
    return filtered
  }, [dailyTask.tasks, dailyTask.id, dailyTask.submitted]) // Added dailyTask.submitted as dependency
  ...
  console.log(`🔍 [DailyTaskGroup] Task analysis:`, {
    filteredTasksCount: filteredDailyTasks.length,
    hasInProgressTasks,
    hasCompletedTask
  })
  ...
  console.log(`🔍 [DailyTaskGroup] Final render decision for daily task ${dailyTask.id}:`, {
    filteredTasksCount: filteredDailyTasks.length,
    shouldRender: filteredDailyTasks.length > 0
  })

  // Only render if there are filtered tasks to show
  if (filteredDailyTasks.length === 0) {
    console.log(`🔍 [DailyTaskGroup] Not rendering - no filtered tasks`)
    return null
  }
    ...
    >
      ...
      <Card>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                {statusCounts.completed && (
                  <Badge 
                    variant="outline" 
                    className={`${!dailyTask.submitted 
                      ? "bg-yellow/10 text-yellow border-yellow" 
                      : "bg-green/10 text-green border-green"}`}
                  >
                    <FaCheckCircle className="mr-1" /> 
                    {statusCounts.completed} Completed{!dailyTask.submitted ? " (Not Submitted)" : ""}
                  </Badge>
                )}
                  ...
                  {isActionsPopupOpen && (
                    ...
                    >
                      <div className="p-2 space-y-2">
                        ...
                        <Button
                          onClick={() => {
                            navigate("/supervisor/tasks/create")
                        ...
                        <Button
                          onClick={() => {
                            navigate("/supervisor/shifted-task")
                            setIsActionsPopupOpen(false)
                          }}
                        ...
                        <Button
                          onClick={() => {
                            navigate("/supervisor/submitted-task")
                            setIsActionsPopupOpen(false)
                          }}
        ...
        <CardContent className="pt-4">
          ...
          {/* Enhanced info alert for today's completed tasks not submitted */}
          {isToday && statusCounts.completed > 0 && !dailyTask.submitted && (
            <div className="mb-4 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
              <div className="flex items-start">
                <FaExclamationCircle className="h-5 w-5 text-yellow-600 mt-0.5 mr-3" />
                <div>
                  <h4 className="font-medium text-yellow-800">Today's Completed Tasks</h4>
                  <p className="text-sm text-yellow-700 mt-1">
                    You have {statusCounts.completed} completed task{statusCounts.completed > 1 ? 's' : ''} from today 
                    that {statusCounts.completed > 1 ? 'are' : 'is'} not submitted yet. These tasks will remain visible 
                    until you submit your daily tasks or until they are reviewed and approved.
                  </p>
                </div>
              </div>
            </div>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            {/* Show special indicators for today's completed tasks */}
                            {isToday && task.status === "completed" && !dailyTask.submitted && (
                              <Badge variant="outline" className="bg-yellow/10 text-yellow border-yellow text-xs">
                                Not Submitted
                              </Badge>
                            )}
                          </div>
                          ...
                          <Tooltip>
                            ...
                            <TooltipContent className="bg-white text-gray-700">
                              ...
                              {task.originalDueDate && task.originalDueDate !== task.due_date && (
                                <p>Original date: {safeParseDate(task.originalDueDate)?.toLocaleDateString() || "Invalid Date"}</p>
                              )}
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
